...
/**
 * GA fix (SEO audit, Sept 2026 — confirmed still broken in production on
 * 2026-09-23 despite a fix for this existing on an unmerged branch,
 * seo-fixes-2026-09-22). This used to `import { GA_MEASUREMENT_ID } from
 * "@/lib/analytics/ga"` — but that module is `"use client"`, and Next
 ...
 * real string, which silently broke GA on every single page load (no JS
 * error visible to a user, but the tag never actually fired). A plain
 * `process.env` read here has no such boundary — `NEXT_PUBLIC_*` vars are
 * inlined at build time regardless of which side reads them, so this is the
 * correct way for a Server Component to get the id. `lib/analytics/ga.ts`

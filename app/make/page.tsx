 ...
 * editor. (Mode-specific prompts land in a later pass; the choice screen is the
 * IA change that matters.)
 */
  ...
  return (
    <main className="mk">
      ...
      <div className="mk-grid">
        {MODES.map((m) => (
          <form key={m.label} action={createStoryAction} className="mk-card">
            <span className="mk-label">{m.label}</span>
            <span className="mk-prompt">{m.prompt}</span>
            <button className="mk-cta" type="submit">{m.cta} →</button>
          </form>
        ))}
      </div>
    </main>
...
.mk-card{ background:var(--paper-2); border:1px solid var(--line); border-radius:16px; padding:1.8rem 1.7rem;
  display:flex; flex-direction:column; gap:.7rem; align-items:flex-start; transition:transform .2s cubic-bezier(.2,1.25,.3,1), border-color .2s; }
...
.mk-cta{ margin-top:.4rem; font-family:var(--f-mono); font-size:.72rem; letter-spacing:.08em; text-transform:uppercase;
  background:var(--ink); color:var(--paper); border:0; border-radius:999px; padding:.7rem 1.1rem; cursor:pointer; transition:background .2s; }
.mk-cta:hover{ background:var(--electric); }
